...
pub(crate) static RE_CMGL: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"\+CMGL:\s*(\d+),(\d+)[^\r\n]*\r?\n([0-9A-Fa-f]+)").unwrap());
pub(crate) static RE_CMGL_HEADER: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"\+CMGL:\s*(\d+),(\d+)").unwrap());
